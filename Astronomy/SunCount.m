...
+(void)sunrisetWithLongitude:(double)longitude andLatitude:(double)latitude andResponse:(void (^)(SunString *))responseBlock
{
    ...
    [sunCount sunrisetWithLongitude:longitude andLatitude:latitude andYear:year andMonth:month andDay:day];
    
    struct tm *gmt = gmtime( &t );
    int zone = hour - gmt->tm_hour + (day - gmt->tm_mday)*24;
    
    //calcute sunrise and sunset time
    hour = sunCount.sunrise + zone;
    minute = (sunCount.sunrise - hour+zone)*60;
    if( hour > 24 ) hour -= 24;
...
-(double)GMST0:(double)d{
    ...
    double sidtim0 = [self revolution:( 180.0 + 356.0470 + 282.9404 ) +
