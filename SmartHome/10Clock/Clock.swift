...
open class TenClock : UIControl{
    let MAX = 14.1808001724537
    let MIN = 1.65806278939448
    let SUSUAN = 6.32681853847933
    var  anim:CGAffineTransform!
    var  endAnim:CGAffineTransform!
    open var delegate:TenClockDelegate?
    //overall inset. Controls all sizes.
    @IBInspectable var insetAmount: CGFloat = 20
    var internalShift: CGFloat = 5;
    var pathWidth:CGFloat = 20
...
//    open var tailBackgroundColor = UIColor.white.withAlphaComponent(0.8)
    ...
    open var headBackgroundColor = UIColor.clear
    open var tailBackgroundColor = UIColor.clear
    ...
    open var endDate: Date{
        get{return endAngleToTime(headAngle) }
...
//            trackLayer.strokeColor = strokeColor.withAlphaComponent(0.1).cgColor
    ...
    func angleToTime(_ angle: Angle) -> Date{
        let a:Double = 4*M_PI/(self.MAX-self.MIN)*Double(angle)
        
        let tmp = CGFloat(-a)-CGFloat(M_PI/2)
        self.anim = CGAffineTransform(rotationAngle: tmp)
//        print("begin-angle:\(angle)")
    ...
    func endAngleToTime(_ angle: Angle) -> Date{
        let a:Double = 4*M_PI/(self.MAX-self.MIN)*Double(angle)
        
        let tmp = CGFloat(-a)-CGFloat(M_PI/2)
        
//        print("end-angle:\(angle)")
        self.endAnim = CGAffineTransform(rotationAngle: CGFloat(tmp))
    ...
    func updateGradientLayer() {
        gradientLayer.backgroundColor = UIColor.gray.withAlphaComponent(0.5).cgColor
...
//        topTailLayer.fillColor = disabledFormattedColor(tailBackgroundColor).cgColor
        ...
        topHeadLayer.addSublayer(stImgLayer)
        topHeadLayer.backgroundColor = UIColor.clear.cgColor
        topTailLayer.addSublayer(endImgLayer)
        topTailLayer.position = tailPoint
        topHeadLayer.position = headPoint
...
//            print("注意，这个也tm是反的！！！！！！")
            //注意，这个也tm是反的！！！！！！
            topHeadLayer.setAffineTransform(self.endAnim)
            topTailLayer.setAffineTransform(self.anim)
        }
    }
