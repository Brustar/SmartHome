...
+ (NSMutableArray *)typeName:(int)typeID byRoom:(int) roomID
{
    ...
    if([db open])
    {
        NSString *sql ;
        if ([self isWholeHouse:roomID]) {
            sql = [NSString stringWithFormat:@"SELECT distinct typeName,htypeid FROM Devices where subtypeid = %d",typeID];
        }else{
            sql = [NSString stringWithFormat:@"SELECT distinct typeName,htypeid FROM Devices where subtypeid = %d and rID = '%d'",typeID,roomID];
        }
        FMResultSet *resultSet = [db executeQuery:sql];
        int i=0;
        while ([resultSet next])
