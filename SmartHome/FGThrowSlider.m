...
+ (FGThrowSlider *)sliderWithFrame:(CGRect)frame delegate:(id <FGThrowSliderDelegate>)del baseTrack:(UIImage *)baseImage thumb:(UIImage *)thumbImage {
    return [[FGThrowSlider alloc] initWithFrame:frame
                                       delegate:del
                                      baseTrack:baseImage
                                          thumb:thumbImage];
...
- (instancetype)initWithFrame:(CGRect)frame delegate:(id <FGThrowSliderDelegate>)del baseTrack:(UIImage *)baseImage thumb:(UIImage *)thumbImage{
    self = [super initWithFrame:frame];
    if (self) {
        if (NSClassFromString(@"UIDynamicAnimator")) {
            animator = [[UIDynamicAnimator alloc] initWithReferenceView:self];
        ...
        CGFloat width = (_rightImage) ? self.frame.size.width-xmargin-20 : self.frame.size.width-xmargin;
        base = [[UIView alloc] initWithFrame:CGRectMake(xmargin, self.frame.size.height/2-1, width, 2)];
        base.backgroundColor = [UIColor colorWithPatternImage:baseImage];
        [self addSubview:base];
